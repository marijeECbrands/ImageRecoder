...
APP_NAME = "Fashion Image Renamer v6.5.1"
CACHE_VERSION = "6.5.1"
...
def visual_match(
    ...
    """Vergelijk target uitsluitend met exacte categorie-7 packshot anchors."""
    candidates = []
    for group_key, anchors in sorted(known_groups.items()):
        for anchor in anchors:
            content = [
                {"type": "input_text", "text": (
                    "Bepaal of de DOELFOTO HETZELFDE KLEDINGARTIKEL bevat als de EXACTE PACKSHOTREFERENTIE. "
                    "Het beeldtype hoeft niet gelijk te zijn: de doelfoto mag een model tonen dat het artikel draagt, "
                    "een zij-/achteraanzicht zijn of slechts een artikel- of stofdetail tonen. "
                    "KLEUR/PRINT IS EEN HARDE GATE: een duidelijk andere kleur of print is altijd NEE; ""normale belichtings-, schaduw- en witbalansverschillen zijn toegestaan. Bij een stofdetail moeten ""zowel materiaal/structuur als kleur/print overeenkomen. ""Vergelijk daarna productidentiteit op silhouet/vorm, lengte, hals/kraag, sluiting, mouwen, zakken, naden, "
                    "constructiedetails, materiaal, kleur en print. Alleen dezelfde kleur/stof is onvoldoende. "
                    "Een ander kledingstuk is NEE. Bij twijfel kies NEE. "
                    'Antwoord uitsluitend als JSON: {"same_article":true,"confidence":0-100,"explanation":"kort"}'
                )},
                {"type": "input_text", "text": "Exacte packshotreferentie:"},
                {"type": "input_image", "image_url": data_url(anchor, ai_max_px)},
                {"type": "input_text", "text": "Doelfoto:"},
                {"type": "input_image", "image_url": data_url(target, ai_max_px)},
            ]
            response=client.responses.create(model=model,input=[{"role":"user","content":content}])
            payload=parse_json(response.output_text)
            same=bool(payload.get("same_article",False))
            confidence=int(payload.get("confidence",0) or 0)
            explanation=str(payload.get("explanation",""))
            if same:
                candidates.append((group_key,confidence,explanation))
    if not candidates:
        return None,0,"Geen exacte packshot-anchor gaf een JA-match."
...
def run_pipeline(
    ...
    # Ruim de tijdelijke v6.5-cache op; vanaf nu is er nog maar één cachebestand.
    legacy_cache = output_dir / "classification_cache_v6_5.json"
    ...
    # Cache is versiegebonden. Oude prompt-/categorielogica wordt nooit hergebruikt.
    cache: dict[str, dict] = {}
    if cache_path.exists():
        try:
            ...
            if (
                ...
                and isinstance(loaded.get("items"), dict)
            ):
                cache = loaded["items"]
        except Exception:
            cache = {}

    with tempfile.TemporaryDirectory(prefix="fashion_v63_") as temp_name:
        work = Path(temp_name)
        all_images = extract_images(image_source, work)
        if not all_images:
            raise ValueError("Geen ondersteunde afbeeldingen gevonden.")

        # v6.5: rapport is altijd uitsluitend de huidige run; nooit oude regels meenemen.
        results: list[Result] = []
                ...
                for future in as_completed(future_map):
                    ...
                    try:
                        ...
                        if is_credit_error(exc):
                            raise ApiCreditsExhausted(
                                "OpenAI API-tegoed is op of de API-quota is bereikt. "
                                "De verwerking is direct gestopt."
                            ) from exc
        ...
        # 3b. Direct gematchte resultaten eerst alleen verzamelen.
        # In v6.3 worden definitieve namen en dubbele-categoriebeslissingen pas centraal gedaan,
        # nadat ook de visueel gematchte foto's bekend zijn.
        pending: list[dict] = []
        ...
        for group_key, files in sorted(matched_groups.items()):
            ...
            if isinstance(output, Exception) or output is None:
                ...
                for image in files:
                    ...
                    # Artikel is bekend, maar zonder beeldcategorie kan geen geldige nieuwe naam worden gemaakt.
                    # Controle_nodig bevat alleen reeds hernoemde bestanden.
            ...
            for image in files:
                ...
                elif confidence < category_threshold:
                    result.status = "CONTROLE_NODIG"
                pending.append({"image": image, "result": result})

        # v6.4: alleen exacte naammatches die als categorie 7 zijn geclassificeerd mogen anchor zijn.
        anchor_groups: dict[str, list[Path]] = {}
        for group_key, files in matched_groups.items():
            output = group_outputs.get(group_key)
            if isinstance(output, Exception) or output is None:
                continue
            _, classified = output
            for ref_image in files:
                item = classified.get(ref_image.name, {})
                try:
                    ref_category = int(item.get("category"))
                except (TypeError, ValueError):
                    ref_category = None
                if ref_category == 7:
                    anchor_groups.setdefault(group_key, []).append(ref_image)
        ...
        def visually_resolve(image: Path):
            ...
            group_key, match_conf, match_explanation = visual_match(
                client, model, image, anchor_groups, work,
                ai_max_px=ai_max_px,
            )
            ...
            category, cat_conf, cat_expl = classify_target_with_references(
                client, model, image, anchor_groups[group_key], categories, ai_max_px
            )
        ...
        # 4b. Ook visueel gematchte resultaten alleen verzamelen.
        for image in unmatched:
            output = visual_outputs.get(image)
            initial_explanation = ambiguous.get(image, "Geen gecombineerde naammatch gevonden.")
            result = Result(source=image, match_method="Visuele match tegen exacte packshot-anchor", explanation=initial_explanation)

            if not anchor_groups:
                result.status = "GEEN_REFERENTIES"
                result.explanation += " Er zijn geen exact op naam gematchte categorie-7-packshots om mee te vergelijken."
                safe_copy(result, image, no_match_dir, image.name)
                results.append(result)
                continue
            ...
            if isinstance(output, Exception) or output is None:
                ...
                # Geen artikel/kleur kunnen vaststellen: originele naam hoort nooit in controle_nodig.
                safe_copy(result, image, no_match_dir, image.name)
                results.append(result)
                continue
            ...
            if category not in categories:
                result.status = "ONGELDIGE_CATEGORIE"
            elif cat_conf < category_threshold:
                result.status = "CONTROLE_NODIG"

            pending.append({"image": image, "result": result})

        # v6.5 routing guard:
        # Zonder betrouwbare Artikel nr. + Kleurcode is er géén artikelmatch.
        # Zulke foto's behouden hun originele naam en horen altijd in geen_match.
        still_pending: list[dict] = []
        for entry in pending:
            r = entry["result"]
            image = entry["image"]
            if not r.article_number or not r.color_code:
                if r.status not in ("API_FOUT", "API_FOUT_VISUELE_MATCH"):
                    r.status = "GEEN_BETROUWBARE_MATCH"
                safe_copy(r, image, no_match_dir, image.name)
                results.append(r)
            else:
                still_pending.append(entry)
        pending = still_pending

        # 5. Centrale post-processing per Artikel nr. + Kleurcode.
        # Dit is de kern van v6.3: directe en visuele matches worden samen beoordeeld voordat
        # definitieve namen worden gemaakt. DUBBELE_CATEGORIE_OF_NAAM bestaat daardoor niet meer.
        grouped_pending: dict[str, list[dict]] = {}
        ungrouped_pending: list[dict] = []
        for entry in pending:
            r = entry["result"]
            if r.article_number and r.color_code:
                grouped_pending.setdefault(f"{r.article_number}|{r.color_code}", []).append(entry)
            else:
                ungrouped_pending.append(entry)

        for group_entries in grouped_pending.values():
            first_for_category: dict[int, dict] = {}

            # Eerst dubbele categorieën inhoudelijk oplossen.
            for entry in group_entries:
                r = entry["result"]
                category = r.category

                if category not in categories or r.status not in ("OK", "OK_VISUELE_MATCH"):
                    continue

                # Categorie 5 en 6 zijn multi-image categorieën: meerdere beelden zijn toegestaan.
                if category in (5, 6):
                    continue

                first_entry = first_for_category.get(category)
                if first_entry is None:
                    first_for_category[category] = entry
                    continue

                first_r = first_entry["result"]
                try:
                    verdict = adjudicate_same_category(
                        client, model,
                        first_entry["image"], entry["image"],
                    ...
                    r.explanation = (
                        f"{r.explanation} | Centrale dubbele-categoriecontrole {vconf}%: {vex}"
                    ).strip(" |")

                    if category == 7 and 8 in categories and decision == "SECOND_IS_BACK":
                        r.category = 8
                        r.category_label = categories.get(8, "")
                        r.category_confidence = max(r.category_confidence, vconf)
                        if 8 in first_for_category:
                            r.status = "DUPLICAAT_BEELDTYPE"
                        else:
                            first_for_category[8] = entry

                    elif category == 7 and 8 in categories and decision == "FIRST_IS_BACK":
                        first_r.category = 8
                        first_r.category_label = categories.get(8, "")
                        first_r.category_confidence = max(first_r.category_confidence, vconf)
                        first_r.explanation = (
                            f"{first_r.explanation} | Centrale herclassificatie naar Artikel back "
                            f"{vconf}%: {vex}"
                        ).strip(" |")
                        first_for_category[7] = entry
                        if 8 in first_for_category and first_for_category[8] is not first_entry:
                            first_r.status = "DUPLICAAT_BEELDTYPE"
                        else:
                            first_for_category[8] = first_entry

                    elif decision == "SAME_VIEW":
                        r.status = "DUPLICAAT_BEELDTYPE"

                    else:
                        r.status = "HERCLASSIFICATIE_NODIG"

                except Exception as exc:
                    r.status = "HERCLASSIFICATIE_NODIG"
                    r.explanation = (
                        f"{r.explanation} | Centrale dubbele-categoriecontrole mislukt: {exc}"
                    ).strip(" |")

            # Daarna definitieve namen maken. Detail artikel/stof mogen meerdere beelden hebben:
            # eerste houdt -5/-6, volgende krijgen -5-2, -5-3 resp. -6-2, -6-3.
            detail_counts: dict[int, int] = {5: 0, 6: 0}
            local_used_names: set[str] = set()
            ...
            for entry in group_entries:
                ...
                if r.status == "DUPLICAAT_BEELDTYPE":
                    safe_copy(r, image, duplicate_dir, image.name)
                    results.append(r)
                    continue

                if category not in categories:
                    # Geen geldige beeldcategorie = geen geldige nieuwe bestandsnaam.
                    safe_copy(r, image, no_match_dir, image.name)
                    results.append(r)
                    continue
                ...
                if category in (5, 6):
                    ...
                    seq = detail_counts[category]
                    category_part = str(category) if seq == 1 else f"{category}-{seq}"
                else:
                    category_part = str(category)

                filename = (
                    f"{r.article_number}-{r.color_code}-{category_part}"
                    f"{image.suffix.lower()}"
                )

                # Een naamconflict na de centrale controle is niet meer een technische dubbele status:
                # het gaat naar handmatige herclassificatie.
                if filename.lower() in local_used_names or filename.lower() in used_names:
                    r.status = "HERCLASSIFICATIE_NODIG"
                    r.explanation = (
                        f"{r.explanation} | Na centrale controle bestaat deze definitieve naam nog steeds al."
                    ).strip(" |")

                local_used_names.add(filename.lower())
                used_names.add(filename.lower())

                if r.status in ("OK", "OK_VISUELE_MATCH") and category == 8:
                    safe_copy(r, image, article_back_dir, filename)
                elif r.status in ("OK", "OK_VISUELE_MATCH"):
                    safe_copy(r, image, good_dir, filename)
                else:
                    # Vanaf v6.5.1 bevat controle_nodig uitsluitend bestanden met een
                    # geldige hernoemde naam; filename is hier al Artikel-Kleur-Categorie.
                    safe_copy(r, image, review_dir, filename)

                results.append(r)

        # Eventuele resultaten zonder artikel/kleur kunnen niet centraal gegroepeerd worden.
        for entry in ungrouped_pending:
            r = entry["result"]
            image = entry["image"]
            r.status = "GEEN_BETROUWBARE_MATCH"
            safe_copy(r, image, no_match_dir, image.name)
    ...
    try:
        cache_path.write_text(json.dumps({"version": CACHE_VERSION, "items": cache}, ensure_ascii=False, indent=2), encoding="utf-8")
    except Exception:
        pass
